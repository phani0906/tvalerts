...
(async function boot() {
  ...
  renderFiveMinTable();
  renderFifteenMinTable();
  renderOneHrTable();a// public/scanner.js
const socket = io();
...
/* Tolerances */
const TOLERANCE = {
  ma20_5m: 0.50, vwap_5m: 0.50, daymid_5m: 1.00,
  ma20_15m: 1.00, vwap_15m: 1.00, daymid_15m: 1.00,
  ma20_1h: 2.00, vwap_1h: 2.00, daymid_1h: 1.00
};
window.TOLERANCE = TOLERANCE;
...
function renderFiveMinTable() {
  ...
  alerts5m.forEach(a => {
    ...
    // header order: Time, Ticker, Alert, Price, MA20(5m), VWAP(5m), DayMid
    let td = document.createElement('td'); td.textContent = a.Time || ''; row.appendChild(td);
    td = document.createElement('td'); td.textContent = a.Ticker || ''; row.appendChild(td);
...
function renderFifteenMinTable() {
  ...
  alerts15m.forEach(a => {
    ...
    // header order: Time, Ticker, Price, Alert, MA20(15m), VWAP(15m), DayMid
    let td = document.createElement('td'); td.textContent = a.Time || ''; row.appendChild(td);
    td = document.createElement('td'); td.textContent = a.Ticker || ''; row.appendChild(td);
    td = document.createElement('td'); td.textContent = fmt2(toNum(p.Price)); row.appendChild(td);
...
function renderOneHrTable() {
  ...
  alerts1h.forEach(a => {
    ...
    // header order: Time, Ticker, Price, Alert, MA20(1h), VWAP(1h), DayMid
    let td = document.createElement('td'); td.textContent = a.Time || ''; row.appendChild(td);
    td = document.createElement('td'); td.textContent = a.Ticker || ''; row.appendChild(td);
    td = document.createElement('td'); td.textContent = fmt2(toNum(p.Price)); row.appendChild(td);
...
/* ===== Sockets ===== */
socket.on('alertsUpdate:AI_5m', rows => {
  console.log('[client] alertsUpdate:AI_5m', Array.isArray(rows) ? rows.length : 0);
  alerts5m = dedupe(Array.isArray(rows) ? rows : [], 'AI_5m');
  renderFiveMinTable();
});
socket.on('alertsUpdate:AI_15m', rows => {
  console.log('[client] alertsUpdate:AI_15m', Array.isArray(rows) ? rows.length : 0);
  alerts15m = dedupe(Array.isArray(rows) ? rows : [], 'AI_15m');
  renderFifteenMinTable();
});
socket.on('alertsUpdate:AI_1h', rows => {
  console.log('[client] alertsUpdate:AI_1h', Array.isArray(rows) ? rows.length : 0);
  alerts1h = dedupe(Array.isArray(rows) ? rows : [], 'AI_1h');
  renderOneHrTable();
});

socket.on('priceUpdate', data => {
  console.log('[client] priceUpdate keys:', Object.keys(data || {}));
  priceData = data || {};
  renderFiveMinTable();
  renderFifteenMinTable();
  renderOneHrTable();
