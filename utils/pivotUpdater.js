...
function classifyCPRByHistory(widths, todayWidth) {
  ...
  const pct    = (rank - 0.5) / sorted.length;              // 0..1

  let cprClass = 'normal';
  if (sorted.length >= 6) {
    const lowCut  = Math.max(2, Math.floor(sorted.length * 0.3));
    const highCut = sorted.length - Math.max(1, Math.floor(sorted.length * 0.3)) + 1;
...
async function fetchDailyMA20(ticker) {
  const period2 = new Date();
  const period1 = new Date(Date.now() - 120 * 24 * 3600 * 1000);
  const rows = await yahooFinance.historical(ticker, { period1, period2, interval: '1d' });
...
async function buildRows(tickers) {
  ...
  for (const t of tickers) {
    ...
    if (dailies?.day1 && dailies?.day2) {
      ...
      pdh            = isNum(dailies.day1.high) ? +dailies.day1.high.toFixed(2) : null; // 👈 PDH
    }
